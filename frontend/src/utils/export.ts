...
export const exportReviewAsPDF = ({
	...
	// y += 6;
	// doc.setFontSize(10);
	// doc.text(`Status: ${existingPolicy.hasExistingPolicy ? "Yes" : "No"}`, 10, y);
	// y += 5;
	// doc.text(
	...
	// Object.entries(existingPolicy.existingPolicyData || {}).forEach(
	// 	([id, policy]: any) => {
	// 		y += 8;
	// 		doc.setFont("", "bold");
	// 		doc.text(`Policy ID: ${id}`, 10, y);
	// 		doc.setFont("", "normal");
	// 		y += 5;
	// 		doc.text(`Policy Name: ${policy.policyName}`, 10, y);
	// 		y += 5;
	// 		doc.text(`Plan Name: ${policy.otherName}`, 10, y);
	// 		y += 5;
	// 		doc.text(
	...
	// 		);
	// 		y += 5;
	// 		doc.text(`Cover Amount: ${policy.coverAmount} lacs`, 10, y);
	// 		y += 5;
	// 		const coverage = Array.isArray(policy.coverage)
	// 			? policy.coverage.map(getLabel).join(", ")
	// 			: getLabel(policy.coverage);
	// 		doc.text(`Members Covered: ${coverage}`, 10, y);
	// 	}
	// );
	doc.setFontSize(14);
	doc.text("4. Existing Policy Details", 10, y);
	...
	policies.forEach(([id, policy]: any, index) => {
		...
		if (col === 0 && index !== 0) {
			y += 50; // height of one card
		}

		// Background box
		doc.setFillColor(241, 245, 249); // Tailwind's slate-100
		...
		tempY += 5;
		doc.text(`Cover Amount: ${policy.coverAmount} lacs`, x + colPadding, tempY);
		tempY += 5;
