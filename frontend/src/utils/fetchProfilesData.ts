...
export type Steps = "profile" | "lifestyle" | "medicalHistory" | "existingPolicy" | "review";

export const fetchProfilesData = (currentStep: Steps) => {
  const navigate = useNavigate();
  const location = useLocation();
  const dispatch = useDispatch();

  const profiles = useSelector((s: RootState) => s.profiles.profileData);
  const _personalInfo = useSelector((s: RootState) => s.personal.personalInfo);
